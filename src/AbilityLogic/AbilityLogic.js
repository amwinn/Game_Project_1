...
function castProjectile(ability, source, dataSet) {
    const size = new Size(ability.form.data_set.size.dw, ability.form.data_set.size.dh); //change to not be hardcoded, will need to refactor tilemap to be outside of mapdb class
    ...
    dataSet.projectilesArray.push(new Projectile(Projectile.playerProjectile, position, size, velocity));
}
